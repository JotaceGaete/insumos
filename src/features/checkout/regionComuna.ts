// Chile's 16 regions and their comunas, kept as a local, versioned dataset —
// no external API/fetch. Official names, with accents. This is the single
// source of truth for both the cascading Región → Comuna selects in the
// checkout UI and (mirrored into a seed table by the Checkout V2 migration)
// server-side validation of the region/comuna pair.
...
export const CHILE_REGIONS: ChileRegion[] = [
  { name: 'Región de Arica y Parinacota', comunas: ['Arica', 'Camarones', 'General Lagos', 'Putre'] },
  { name: 'Región de Tarapacá', comunas: ['Iquique', 'Alto Hospicio', 'Pozo Almonte', 'Camiña', 'Colchane', 'Huara', 'Pica'] },
  { name: 'Región de Antofagasta', comunas: ['Antofagasta', 'Mejillones', 'Sierra Gorda', 'Taltal', 'Calama', 'Ollagüe', 'San Pedro de Atacama', 'Tocopilla', 'María Elena'] },
  { name: 'Región de Atacama', comunas: ['Copiapó', 'Caldera', 'Tierra Amarilla', 'Chañaral', 'Diego de Almagro', 'Vallenar', 'Alto del Carmen', 'Freirina', 'Huasco'] },
  { name: 'Región de Coquimbo', comunas: ['La Serena', 'Coquimbo', 'Andacollo', 'La Higuera', 'Paiguano', 'Vicuña', 'Illapel', 'Canela', 'Los Vilos', 'Salamanca', 'Ovalle', 'Combarbalá', 'Monte Patria', 'Punitaqui', 'Río Hurtado'] },
  {
  ...
  },
  {
    ...
    comunas: [
      'Chillán', 'Chillán Viejo', 'Bulnes', 'Cobquecura', 'Coelemu', 'Coihueco', 'El Carmen', 'Ninhue', 'Ñiquén',
...
export function listComunasForRegion(regionName: string): string[] {
  return CHILE_REGIONS.find((region) => region.name === regionName)?.comunas ?? [];
}

export function isValidRegion(regionName: string): boolean {
  return CHILE_REGIONS.some((region) => region.name === regionName);
}

export function isValidRegionComuna(regionName: string, comunaName: string): boolean {
  const region = CHILE_REGIONS.find((entry) => entry.name === regionName);
  return region ? region.comunas.includes(comunaName) : false;
}

...
// unavailable, missing fields, quantity out of range). Only messages
// matching those known shapes are shown as-is; anything else — a Postgrest
// error (missing function, constraint violation, connection issue), a raw
// column name, whatever — falls back to a generic message so internal
// Supabase details never reach the customer.
 ...
 * Nothing here — price, stock, product/variant name — is trusted from the
 * caller; only variantId + quantity + the buyer's own contact/shipping data
 * are sent.
 */
